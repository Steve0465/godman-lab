...
async def test_install_all_with_npm_packages():
    ...
    with patch('godman_ai.diagnostics.installer._check_command_available', new_callable=AsyncMock) as mock_check:
        with patch('godman_ai.diagnostics.installer._run_command', new_callable=AsyncMock) as mock_cmd:
...
async def test_run_llm_health_check_basic():
    ...
    with patch('godman_ai.diagnostics.llm_health._check_openai_health', new_callable=AsyncMock) as mock_openai:
        with patch('godman_ai.diagnostics.llm_health._check_google_ai_health', new_callable=AsyncMock) as mock_google:
...
async def test_run_llm_health_check_with_external_commands():
    ...
    with patch('godman_ai.diagnostics.llm_health._check_openai_health', new_callable=AsyncMock) as mock_openai:
        with patch('godman_ai.diagnostics.llm_health._check_google_ai_health', new_callable=AsyncMock) as mock_google:
            with patch('godman_ai.diagnostics.llm_health._run_external_health_check', new_callable=AsyncMock) as mock_external:
